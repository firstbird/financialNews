...
class ReplyNoticeBloc  extends Bloc<ReplyNoticeEvent, ReplyNoticeState> {
  ...
  ReplyNoticeBloc() : super(initState()) {
    on<initStateNoticeAndReply>((event, emit) async {
      emit(PostInitial());
    });
    on<getUserCommentReplyNotice>((event, emit) async {
      ...
      if((count != null && count >= 0) || getLikeCount >=0){
        emit(newReplyCount(count: count + getLikeCount+newlithumbupCount , followcount: getLikeCount));
      }
...
//      }
    ...
    on<readed>((event, emit) async {
      ...
      if((count != null && count >= 0) || getLikeCount >=0){
        emit(newReplyCount(count: count + getLikeCount + newlithumbupCount, followcount: getLikeCount, newlithumbupCount: newlithumbupCount));
        return;
      }
    ...
    on<OrderExpiration>((event, emit) async {
