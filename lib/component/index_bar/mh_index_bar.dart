...
/// IndexBar touch callback IndexModel.
typedef void IndexBarTouchCallback(IndexBarDragDetails model);
      ...
      child: _IndexBar(
        ...
        onTouch: (details) {
          if (widget.onTouch != null) {
            if (_isTouchDown != (details.action == IndexBarDragDetails.actionDown)) {
              setState(() {
                _isTouchDown = details.action == IndexBarDragDetails.actionDown;
              });
            }
...
class _IndexBar extends StatefulWidget {
  ...
  _IndexBar({
    ...
    required this.mapSelTag,
    required this.mapHintTag,
    required this.onTouch,
    this.itemWidth = 30,
    this.itemHeight = 16,
    this.tagWidth = 14,
...
class _IndexBarState extends State<_IndexBar> {
  late List<int> _indexSectionList;
  int _widgetTop = -1;
  int _lastIndex = 0;
  bool _widgetTopChange = false;
  bool _isTouchDown = false;
  IndexBarDragDetails _indexModel = new IndexBarDragDetails();
  ...
  bool _fetchOffstage(String tag) {
    if (_indexModel.tag == tag) {
      final List<String> ignoreTags = widget.ignoreTags ?? [];
      return ignoreTags.indexOf(tag) != -1 ? true : !(_indexModel.action == IndexBarDragDetails.actionDown);// todo mzl contact
    }
    return true;
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.tag != null &&
      ...
      // todo mzl contact
      _indexModel.action = IndexBarDragDetails.actionUp;
      _indexModel.index = widget.data.indexOf(widget.tag);
    ...
    return GestureDetector(
      onVerticalDragDown: (DragDownDetails details) {
        if (_widgetTop == -1 || _widgetTopChange) {
          ...
          Offset topLeftPosition = (box as RenderBox).localToGlobal(Offset.zero);
        ...
        if (index != -1) {
          _lastIndex = index;
          // todo mzl contact
          _indexModel.action = IndexBarDragDetails.actionDown;
          _indexModel.index = index;
