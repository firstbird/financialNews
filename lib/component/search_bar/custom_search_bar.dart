...
mixin _ControllerListener<T> on State<MySearchBar<T>> {
...
class CustomSearchBarController<T> {
  final List<T> _list = [];
...
class MySearchBar<T> extends StatefulWidget {
  ...
  MySearchBar({
    ...
    this.isFocusOnShow = false,
    this.isCustList = false,
    @required this.textInputType = TextInputType.text
  }) : super(key: key);

  @override
  _MySearchBarState createState() => _MySearchBarState<T>();
}

class _MySearchBarState<T> extends State<MySearchBar<T?>>
  ...
  bool _animate = false;
  List<T?> _list = [];
  late CustomSearchBarController searchBarController;
  ...
  _onTextChanged(String newText) async {
    if (_debounce?.isActive ?? false) {
      print("[custom search bar] ontextChanged debounce cancel");
      _debounce!.cancel();
    }

    _debounce = Timer(widget.debounceDuration, () async {
      if (newText.length >= widget.minimumChars && widget.onSearch != null) {
        print("[custom search bar] searchBarController._search");
        searchBarController._search(newText, widget.onSearch);
      } else {
        print("[custom search bar] clear");
        setState(() {
          _list.clear();
          _error = null;
          _loading = false;
              ...
              children: <Widget>[
                Flexible(
                  child: AnimatedContainer(
                    ...
                    width: widthMax * .8,// _animate ? widthMax * .8 : widthMax,
