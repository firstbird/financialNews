...
import 'package:recipe/component/app_bar/mh_app_bar.dart';
import 'package:recipe/component/index_bar/mh_index_bar.dart';
import 'package:recipe/component/list_tile/mh_list_tile.dart';
import 'package:recipe/component/search_bar/search_bar.dart';
import 'package:recipe/constant/constant.dart';
import 'package:recipe/constant/style.dart';
import 'package:recipe/providers/tab_bar_provider.dart';
import 'package:recipe/repository/data/user.dart';
import 'package:recipe/utils/service/contacts_service.dart';
import 'package:recipe/widgets/mainframe/search_content.dart';
...
class _ContactsPageState extends State<ContactsPage>
  ...
  /// 悬浮view 高度 向上取整
  int _suspensionHeight = //FlutterScreenUtil.ScreenUtil(); // todo mzl contact
      (ScreenUtil().setHeight(99.0) as double).ceil();

  /// 每个item 高度 向上取整
  int _itemHeight = (ScreenUtil().setHeight(168.0) as double).ceil(); // todo mzl contact
  ...
  Widget _buildSusWidget(BuildContext context, int index) {
    // todo mzl contact
    bool isFloat = false;
    String susTag = _suspensionTag;
    return Container(
  ...
  Widget _buildListItem(User user) {
    String susTag = "test tag"; // todo mzl contact user.getSuspensionTag();
    return Column(
      children: <Widget>[
        Offstage(
          offstage: false, //todo mzl contact user.isShowSuspension != true
          child: _buildSusWidget(context, 0), //todo mzl contact
        ),
        Container(
          ...
          child: Column(
            children: <Widget>[
              Expanded(
                // todo mzl contact user.profileImageUrl user.screenName
                child: _buildItem("https://p1.music.126.net/g2_Gv0dtAicJ3ChTYu28_g==/1393081239628722.jpg", "test screen name", true,
        ...
        // 底部显示共有多人
        Offstage(
          offstage: false, //  todo mzl contact _lastContact.idstr != user.idstr
          child: Container(
  ...
  Widget _buildItem(
    ...
    final double iconWH = ScreenUtil().setWidth(120.0);
    // 头部分
    Widget leading = Padding(
      padding: EdgeInsets.only(right: ScreenUtil().setWidth(39.0)),
      child: Container(
        decoration: BoxDecoration(
    ...
    Widget listTile = MHListTile(
      ...
      middle: middle,
      height: _itemHeight.toDouble(),
      dividerIndent: ScreenUtil().setWidth(208.0),
    ...
    return Slidable(
      ...
      // controller: _slidableController, //  todo mzl contact
      //  todo mzl contact
  ...
  Widget _buildDefaultIndexBarList() {
    return AzListView(
      ...
      data: _contactsList,
      itemBuilder: (context, model) => _buildListItem(model as User),
      susItemBuilder: _buildSusWidget,// todo mzl contact _buildSusWidget(_suspensionTag, isFloat: true),
      // isUseRealIndex: true, //  todo mzl contact
      indexBarItemHeight: _itemHeight.toDouble(), //  todo mzl contact
      susItemHeight: _suspensionHeight.toDouble(), //  todo mzl contact
      // onSusTagChanged: _onSusTagChanged, //  todo mzl contact
  ...
  Widget _buildCustomIndexBarList({bool builderMode = true}) {
    return AzListView(
      ...
      data: _contactsList,
      itemBuilder: (context, model) => _buildListItem(model as User),
      susItemBuilder: _buildSusWidget,// todo mzl contact //suspensionWidget: _buildSusWidget(_suspensionTag, isFloat: true),
      // isUseRealIndex: true, //  todo mzl contact
      indexBarItemHeight: _itemHeight.toDouble(), //  todo mzl contact itemHeight: _itemHeight,
      susItemHeight: _suspensionHeight.toDouble(), //  todo mzl contact suspensionHeight: _suspensionHeight,
      // onSusTagChanged: _onSusTagChanged, todo mzl contact
      // header: AzListViewHeader(
      //   // - [特殊字符](https://blog.csdn.net/cfxy666/article/details/87609526)
      //   // - [特殊字符](http://www.fhdq.net/)
  ...
  Widget _buildIndexBarTagWidget(
      BuildContext context, String tag, IndexBarDragDetails indexModel) {
    return Container(
      alignment: Alignment.center,
      decoration: BoxDecoration(
  ...
  /// 获取背景色
  Color _fetchColor(String tag, IndexBarDragDetails indexModel) {
    Color color;
  ...
  /// 构建某个tag
  Widget _buildTagWidget(String tag, IndexBarDragDetails indexModel) {
    Color textColor;
    Color selTextColor;
    if (INDEX_DATA_0.indexOf(tag) != -1) {
      // 浅黑 Color(0xFF555555)
  ...
  /// 构建某个hint中子部件
  Widget _buildHintChildWidget(String tag, IndexBarDragDetails indexModel) {
    if (tag == '♀') {
      // 返回映射高亮的部件
      return new SvgPicture.asset(
        Constant.assetsImagesSearch + 'icons_filled_search.svg',
  ...
  // 获取Offstage 是否隐居幕后
  bool _fetchOffstage(String tag, IndexBarDragDetails indexModel) {
    if (indexModel.tag == tag) {
      final List<String> ignoreTags = [];
      return ignoreTags.indexOf(tag) != -1 ? true : !(indexModel.action == IndexBarDragDetails.actionDown); // todo mzl contact indexModel.isTouchDown
    }
    return true;
  }
}
