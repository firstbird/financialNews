...
class ApplyActivity extends StatefulWidget {
  double mincost = 0.00;
  double maxcost = 0.00;
  Activity? _activity;
  ...
  ApplyActivity({required this.arguments}){
    if(arguments != null){
      _activity = (arguments as Map)["activity"];
  ...
  @override
  _ApplyActivityState createState() => _ApplyActivityState();
}

class _ApplyActivityState extends State<ApplyActivity> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('申请交友', style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.bold),),
        centerTitle: true,
      ),
      body: new MyLoadingProgress(loading: _loading, isNetError: false, child: Container(
        ...
        child: ListView(
          ...
          children: <Widget>[
            buildActivityinfo(),
            SizedBox(height: 6,),
            Text('上传个人照片', style: TextStyle(color: Colors.black, fontSize: 16),),
            buildGridView(),
            SizedBox(height: 9,),
            buildLocation(),
            buildBaseInfo(),
  ...
  //学历和收入认证
  Container buildCertificateInfo(){
    return Container(
      color: Colors.white,
      child: ListTile(
        onTap: () async {
        ...
        onTap: () async {
          ...
          Navigator.pushNamed(context, '/ListViewProvince', arguments:{"isPermanent":true}).then((dynamic value) async {
            if(value != null){
              ...
              print("[apply activity] select city, citycode: ${citycode}, provinceCode: ${provinceCode}, name: ${name}");
              setState(() {
                  _addresstitle = name;
                  _address = name;
                  _city = citycode;
        ...
        title: Container(
          child: Row(
            ...
            children: <Widget>[
              ...
              SizedBox(width: 15,),
              Expanded(
                child: Text((_addresstitle!=null && _addresstitle.isNotEmpty)?_addresstitle:'选择您的所在城市',
  ...
  Container buildIssuedButton(BuildContext context) {
    return Container(
        ...
        child: TextButton(
            ...
            child: Text(
              '确认',style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Colors.white),
            ),
              ...
              try{
                if(_isButtonEnable) {
                  ...
                  if (_textContentController.text == "") {
                    ShowMessage.showToast("亲，您还没介绍自己呢~");
                    return;
                  }
                  ...
                  await addMemberAndJoinActivity();
  ...
  Future<void> getActivityInfo() async {
    ...
    if (widget._activity == null) {
      ShowMessage.showToast("活动已经被删除了");
      setState(() {
  ...
  // todo mzl 申请交友提交后，进入和平台客服的聊天窗口,收到请耐心等待的消息
  Future<void> addMemberAndJoinActivity() async {
    //如果已经加入，就直接进入群聊
    GroupRelation? groupRelation = await imhelper.getGroupRelationByGroupid(Global.profile.user!.uid, widget._activity!.actid);
    if(groupRelation != null && groupRelation.isnotservice == 0){
      print("[applyActivity] joinActivity isnotservice == 0 /MyMessage：-----------------------------------");
      Navigator.pushNamed(context, '/MyMessage', arguments: {"GroupRelation": groupRelation}).
      then((value) => getActivityInfo());
      ShowMessage.showToast('您已提交申请');
      return;
    }
    ...
    if(widget._activity!.status != 0){
      ShowMessage.showToast('活动已结束');
      return;
    }

    bool res = await _activityService.addMember(
        _user.uid,
        _user.token!,
        widget._activity!.actid,
        _provinceCode,
        _city,
        _textContentController.text,
        _imagesUrl,
        widget.goodpriceid,
        "", _paytype, (String statusCode, String msg){
              print("[apply activity] statusCode: ${statusCode}, msg: ${msg}");
              if(statusCode == "-1008"){
                loadingBlockPuzzle(context);
              }
              else if(int.parse(statusCode) < 0){
    ...
    if (!res) {
      print("[apply activity] failed user.uid: ${_user.uid}, actid: ${widget._activity!.actid}-----------------------------------");
      ShowMessage.showToast('您已提交的资料有误，请检查');
      return;
    }

    groupRelation = await _activityService.joinActivity(
        widget._activity!.actid, Global.profile.user!.uid, Global.profile.user!.token!,
    ...
    if(groupRelation != null){
      ...
      await imhelper.updateGroupRelationIsNotService(widget._activity!.actid);
      if(ret > 0){
        //服务器传回的grouprelation再重新获取下，有些属性可能为空
        groupRelation = await imhelper.getGroupRelationByGroupid(Global.profile.user!.uid, widget._activity!.actid);
      }
      ...
      if(ret > 0){
        ShowMessage.showToast('申请成功，跳转到平台客服');
        print("[applyActivity] joinActivity ret > 0 /MyMessage：-----------------------------------");
        Navigator.popAndPushNamed(context, '/MyMessage', arguments: {"GroupRelation": groupRelation, "millisecond": DateTime.now().millisecond}).
        then((value) => getActivityInfo());
        return;
      }
      else{
        ShowMessage.showToast('申请失败，请退出重试');
        return;
      }
    }
  }
