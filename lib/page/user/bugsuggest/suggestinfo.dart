...
class _SuggestInfoState extends State<SuggestInfo> {
  ...
  getSuggest() async {
    _suggest = await imService.getSuggestInfo(Global.profile.user!.uid, Global.profile.user!.token!, widget.suggestid, errorCallBack);
    if(_suggest != null) {
      listComments = await imService.getSuggestCommentList(
          widget.suggestid, Global.profile.user!.uid, errorCallBack);
      await imhelper.selBugAndSuggestState(widget.suggestid, Global.profile.user!.uid, 1, (List<String> actid){
        if(actid.length > 0)
          _islike = true;
      });
        ...
        children: <Widget>[
          ...
          Expanded(
            child:           Column(
              ...
              children: <Widget>[
                Text(_suggest!.user!.username, style: TextStyle(color: Colors.black87, fontSize: 16, fontWeight: FontWeight.bold)),
                Text(CommonUtil.datetimeFormat(DateTime.parse(_suggest!.createtime))),
              ],
            ),
          )
        ],
    ...
    return SafeArea(
      child: Container(
          ...
          child: Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: <Widget>[
              Text(_suggest!.content, style: TextStyle(color: Colors.black87, fontSize: 14, ),),
            ],
          )
      ),
    );
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      if(_isCommentLike) {
                                        ...
                                        if (v.likeuid == 0) {
                                          bool ret = await imService.updateSuggestCommentLike(v.commentid!, Global.profile.user!.uid,
                                            Global.profile.user!.token!, v.user!.uid, widget.suggestid, errorCallBack);
                                          if(ret){
                                          ...
                                          bool ret = await imService.delSuggestCommentLike(v.commentid!, Global.profile.user!.uid,
                              ...
                              items: <PopupMenuEntry>[
                                PopupMenuItem(
                                  child: InkWell(
                                    child: Text('删除'),
                                    onTap: () async {
                                      bool ret = await imService.delMessageSuggest( Global.profile.user!.token!, Global.profile.user!.uid,
                                          v.commentid!, widget.suggestid, errorCallBack);
                                      if(ret){
                                        listComments = await imService.getSuggestCommentList(widget.suggestid, Global.profile.user!.uid, errorCallBack);
                                        if(listComments != null && listComments.length > 0){
                                          listComments = sortComment(listComments, _ordertype);
                                        }
                                        setState(() {
            ...
            children: <Widget>[
              Column(
                ...
                children: <Widget>[
                  IconButton(
                    ...
                    onPressed: () async {
                      if(_isLikeEnter) {
                        _isLikeEnter = false;
                        if (!_islike){
                          if(await imService.updateSuggestLike(widget.suggestid, Global.profile.user!.uid,
                          ...
                          if(await imService.delSuggestLike(widget.suggestid, Global.profile.user!.uid, Global.profile.user!.token!, errorCallBack)){
                            setState(() {
                              _suggest!.likenum = _suggest!.likenum-1;
                              _islike = false;
                            });
                  ...
                  Text(_suggest == null ? "0" : _suggest!.likenum.toString(),
                  ...
                  TextButton(
                    ...
                    onPressed: () async {
                      if (_message.isNotEmpty) {
                        if(commentid == 0){
                          Navigator.pop(context);
                          int retcommentid = await imService.updateSuggestMessage(widget.suggestid, Global.profile.user!.uid,
                          ...
                          Navigator.pop(context);
                          int temreplyid = await imService.updateSuggestCommentReply(commentid, widget.suggestid, Global.profile.user!.uid,
                              Global.profile.user!.token!, touid, _message, "", errorCallBack);
                          if(temreplyid > 0){
                            listComments.forEach((e){
                              if(e.commentid == commentid){
                                ...
                                e.replys!.add(CommentReply(temreplyid, commentid, Global.profile.user,
                                    touser, _message, DateTime.now().toString(), false, widget.suggestid, false, "", "", 0, ""));
                              }
                            });
                            setState(() {
                    ...
                    items: <PopupMenuEntry>[
                      PopupMenuItem(
                        child: InkWell(
                          child: Text('删除'),
                          onTap: () async {
                            bool ret = await imService.delMessageReplySuggest( Global.profile.user!.token!,
                                Global.profile.user!.uid, v.replyid!, widget.suggestid, errorCallBack);
                            if(ret){
                              listComments = await imService.getSuggestCommentList(widget.suggestid, Global.profile.user!.uid, errorCallBack);
                              if(listComments != null && listComments.length > 0){
                                listComments = sortComment(listComments, _ordertype);
