...
class PageFollow extends ConsumerStatefulWidget  {
  const PageFollow({super.key});

  @override
  PageFollowState  createState() => PageFollowState();
}

class PageFollowState extends ConsumerState<PageFollow>
  ...
  Widget getRow(int i) {
    var p = _itemList[i];
    return _FollowItemView(playlist: p, width: 60, type: 0);
  }
  Widget _contentList() {
      ...
      appBar: CustomAppBar(
          title: !isInSearch
            ...
            // todo mzl search area style
                searchWord == "" ? context.strings.follow : searchWord,
                style: TextStyle(
                  color: Colors.white,
                ),
              )
...
class _FollowItemView extends ConsumerWidget {
  const _FollowItemView({
    super.key,
